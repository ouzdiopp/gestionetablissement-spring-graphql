...
import sn.ouznoreyni.service.ProfesseurQueryService;
...
/**
 * REST controller for managing {@link sn.ouznoreyni.domain.Professeur}.
 */
@RestController
...
public class ProfesseurResource {
    ...
    private final ProfesseurQueryService professeurQueryService;

    public ProfesseurResource(
        ProfesseurService professeurService,
        ProfesseurRepository professeurRepository,
        ProfesseurQueryService professeurQueryService
    ) {
        this.professeurService = professeurService;
        this.professeurRepository = professeurRepository;
        this.professeurQueryService = professeurQueryService;
    }
    ...
    public ResponseEntity<ProfesseurDTO> createProfesseur(@Valid @RequestBody ProfesseurDTO professeurDTO) throws URISyntaxException {
        ...
        professeurDTO = professeurService.save(professeurDTO);
        return ResponseEntity.created(new URI("/api/professeurs/" + professeurDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, professeurDTO.getId().toString()))
            .body(professeurDTO);
    }
    ...
    public ResponseEntity<ProfesseurDTO> updateProfesseur(
        ...
        professeurDTO = professeurService.update(professeurDTO);
    ...
    public ResponseEntity<List<ProfesseurDTO>> getAllProfesseurs(
        ProfesseurCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to get Professeurs by criteria: {}", criteria);

        Page<ProfesseurDTO> page = professeurQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /professeurs/count} : count all the professeurs.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countProfesseurs(ProfesseurCriteria criteria) {
        log.debug("REST request to count Professeurs by criteria: {}", criteria);
        return ResponseEntity.ok().body(professeurQueryService.countByCriteria(criteria));
    }
